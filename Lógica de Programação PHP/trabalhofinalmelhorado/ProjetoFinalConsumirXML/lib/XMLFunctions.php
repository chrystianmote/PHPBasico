...
function removerNoticiaXML($id, $caminho) {
    if (file_exists($caminho)) {
        $elementoXML = simplexml_load_file($caminho);
        $removeu = FALSE;
//se o arquivo não tem nenhum elemento salvo
        if ($elementoXML->count() == 0) {
            return FALSE;
        } else {
            $noticia = $elementoXML->children();
//percorre todos os elementos filho do arquivo (children)
            for ($i = 0; $i < count($noticia); $i++) {
                if ($id == $noticia[$i]->idNoticia) {
                    unset($noticia[$i]);
                    $removeu = TRUE;
                    break;
                }
            ...
            if ($removeu) {
                $elementoXML->saveXML($caminho);
                ?>
                <script>
                    var resposta = window.confirm("Notícia Removida com Sucesso! Deseja Voltar as Notícias?");
                    if (resposta) {
                ...
                die("<h3>Arquivo Não Removido</h3>");
            }
        }
    } else {
//Mata o processamento enviando uma mensagem de erro
